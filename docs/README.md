...
The directory ships its own `.gitignore`, so nothing to add to yours. (To share `.unwritten/config.json` with your team, commit it with `git add -f`.)
...
Any other stdio MCP client: `dotnet` with `["tool", "execute", "Unwritten", "--yes", "--", "mcp"]`.

**Through an MCP gateway** (e.g. [McpOrchestrator](https://github.com/Byggarepop/dotnet-mcp-orchestrator)):
the router's LLM only sees your capability description, so give it one that says *when* to call Unwritten. Copy this as the capability's instructions:
...
- **`--claude-code`** adds a [Stop hook](https://docs.claude.com/en/docs/claude-code/hooks) to the repo's `.claude/settings.json`: whenever the agent finishes a turn
...
dropped — and `check --strict` overrides them. Ignores live in `.unwritten/ignores.json` (machine-managed; keep your hands in `config.json`).
...
The first index build drops a fully commented template at `.unwritten/config.json`, so the settings are discoverable right where they
...
Floors take effect immediately. Training settings (`minSupport`, `maxTransactionSize`, `maxExamplesPerPair`, facet training settings) describe
...
least 10 times and `wilson_lb(co(A,B), count(A)) ≥ floor`. The index lives in `.unwritten/index.json`, keyed by the indexed HEAD; when HEAD moves, only the new
commits are counted.

Internally the engine is domain-neutral — it counts *entities* in *transactions*, and git is just the first adapter mapping commits to
...
non-predictive (e.g. a description-only edit to a manifest whose `version` key drives the coupling), the hole is *suppressed*: agents receive it with `suppressed: true` plus the per-key evidence and may overrule; the CLI
prints one summary line and excludes it from the exit code (`--strict` restores the old behavior).
...
File-level rules can only say "you changed `OrderService.cs` but not `OrderServiceTests.cs`". With `"memberLevel": true`, Unwritten learns the same
...
**How it works.** For every commit in the history window, each changed `.cs` file is diffed *syntactically* (Roslyn, no compilation needed) to find which
members actually changed. Those members form the commit's transaction in a
second index (`.unwritten/members.json`), with ids like `Namespace.Type.Method/arity`, scored with the same Wilson statistics as
...
file-level holes for cosmetic C# edits (`suppressed: true`, CLI `--strict` overrides) — whether or not member indexing is enabled.
...
statistics being honest, not a bug. `expected_companions` and `explain_rule` accept member ids directly (auto-detected).
...
- **Method-level (member) rules are C#-only, opt-in, and support-hungry.** Repos need methods changed ≥10 times inside the history window before
