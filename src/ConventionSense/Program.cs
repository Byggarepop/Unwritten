...
switch (args.FirstOrDefault())
{
    ...
    default:
        Console.Error.WriteLine("""
            ConventionSense — git-history-based hole detector.

            Usage:
              conventionsense mcp                     Run as MCP server over stdio.
              conventionsense check [options] [files] Check files for co-change holes.
              conventionsense stats [--repo <path>]   Show index health and rule counts.
              conventionsense reindex [--repo <path>] Rebuild the index from full history.
            ...
            Defaults can be overridden per repo in .conventionsense/config.json.
            """);
        return 2;
}
...
async Task RunMcpServerAsync(string[] mcpArgs)
{
    ...
    builder.Services
        ...
        .WithTools<ConventionSenseTools>();
