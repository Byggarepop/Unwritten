...
public sealed class SyntheticRepo : IDisposable
{
    ...
    public SyntheticRepo()
    {
        Path = System.IO.Path.Combine(
            System.IO.Path.GetTempPath(), "conventionsense-tests", Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(Path);
        Git("init", "--initial-branch=main");
        Git("config", "user.name", "ConventionSense Tests");
        Git("config", "user.email", "tests@conventionsense.invalid");
